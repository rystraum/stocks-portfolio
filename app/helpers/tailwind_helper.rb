module TailwindHelper
  def header_link_to(text, path, **args)
    ...
    if is_active_path
      css_class = "bg-gray-900 text-white px-3 py-2 rounded-md text-sm font-medium"
      args[:aria] ||= {}
      args[:aria][:current] = "page"
    else
      css_class = "text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium"
    end
